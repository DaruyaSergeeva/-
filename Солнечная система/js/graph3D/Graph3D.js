class Graph3D {
    ...
    animateMatrix(x1, y1, z1, key, alpha2, x2, y2, z2) {
        this.math.transformMatrix([
            this.math.moveMatrix(x1, y1, z1),
            this.math[`${key}Matrix`](alpha2),
            this.math.moveMatrix(x2, y2, z2)
        ]);
    }

    twoAnimateMatrix(resultmovematrix1, resultrotatematrix, resultmovematrix2) {
        this.math.transformMatrix([
            resultmovematrix1,
            resultrotatematrix,
            resultmovematrix2
    ...
    calcСorner(subject, endPoint) {
        ...
        for (let i = 0; i < subject.polygons.length; i++) {
            ...
            subject.polygons[i].visible = this.math.calcСorner(viewVector, vector3) <= perpendicular;
        }
    }
    ...
    getProection(point) {
        ...
        const cosa = this.math.calcСorner(this.P1P2, P2M);
        const cosb = this.math.calcСorner(this.P2P3, P2M);
